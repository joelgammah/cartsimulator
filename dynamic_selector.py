...
def find_best_button_xpath(driver, keywords, threshold=0.5):
    ...
    for btn in buttons:
        ...
        if not text:
            continue
        # Log each candidate's text for debugging.
        logger.debug(f"Evaluating button with text: '{text}'")
        for kw in keywords:
            ratio = difflib.SequenceMatcher(None, text, kw.lower()).ratio()
            logger.debug(f"Comparing with keyword '{kw}', ratio: {ratio}")
            if ratio > best_score:
                best_score = ratio
                best_button = btn
    ...
    if best_button is not None and best_score >= threshold:
        ...
        logger.info(f"Dynamic selector chose element with XPath: {xpath} (score: {best_score})")
        return xpath, best_button, best_score
    else:
        logger.info(f"No candidate button reached the threshold. Best score was: {best_score}")
